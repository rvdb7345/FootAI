...
planned_matches = match_df[match_df['Round Number'] == 'Final']
match_df = match_df[match_df['Round Number'] == 'Final']
...
for idx, row in qualification_rounds.iterrows():
    ...
    if len(top_players_home_team) < num_of_top_players:
        historic_players_home_team = historic_player_df[historic_player_df['nationality'] == row[
            ...
            by=['international_reputation', 'overall'], ascending=False).head(num_of_top_players-len(top_players_home_team))
    ...
    if len(top_players_away_team) < num_of_top_players:
        historic_players_away_team = historic_player_df[historic_player_df['nationality'] == row[
            ...
            by=['international_reputation', 'overall'], ascending=False).head(num_of_top_players - len(top_players_away_team))
    ...
    if len(top_players_home_team) == num_of_top_players and len(top_players_away_team) == num_of_top_players:
        ...
        X.loc[idx_number, ['average_international_home', 'average_international_away']] = \
            [top_players_home_team['international_reputation'].mean(), top_players_away_team['international_reputation'].mean()]

        X.loc[idx_number, ['compared_overall_score']] = top_players_home_team['overall'].mean() - top_players_away_team['overall'].mean()
        X.loc[idx_number, ['compared_international']] = top_players_home_team['international_reputation'].mean() - top_players_away_team['international_reputation'].mean()
        ...
        X.loc[idx_number, ['compared_passing']] = top_players_home_team.nlargest(5, 'passing')['passing'].mean() - top_players_away_team.nlargest(5, 'passing')['passing'].mean()
        ...
        X.loc[idx_number, ['gk_compared']] = top_players_home_team[['goalkeeping_diving', 'goalkeeping_handling', 'goalkeeping_kicking',
                          'goalkeeping_positioning', 'goalkeeping_reflexes']].max(axis=0).mean() - top_players_away_team[['goalkeeping_diving', 'goalkeeping_handling', 'goalkeeping_kicking',
                          'goalkeeping_positioning', 'goalkeeping_reflexes']].max(axis=0).mean()
...
for idx, row in match_df.iterrows():
    ...
    if len(top_players_home_team) == num_of_top_players and len(top_players_away_team) == num_of_top_players:
        ...
        X_predict.loc[idx_number, ['compared_overall_score']] = top_players_home_team['overall'].mean() - top_players_away_team['overall'].mean()
        X_predict.loc[idx_number, ['compared_international']] = top_players_home_team['international_reputation'].mean() - top_players_away_team['international_reputation'].mean()
        X_predict.loc[idx_number, ['compared_dribbling']] = top_players_home_team.nlargest(3, 'dribbling')['dribbling'].mean() - \
        top_players_away_team.nlargest(3, 'dribbling')['dribbling'].mean()
        X_predict.loc[idx_number, ['compared_passing']] = top_players_home_team.nlargest(5, 'passing')['passing'].mean() - top_players_away_team.nlargest(5, 'passing')['passing'].mean()
        X_predict.loc[idx_number, ['compared_shooting']] = top_players_home_team.nlargest(2, 'shooting')['shooting'].mean() -\
                                                   top_players_away_team.nlargest(2, 'shooting')['shooting'].mean()
        X_predict.loc[idx_number, ['compared_defending']] = top_players_home_team.nlargest(2, 'defending')['defending'].mean() - \
                                                    top_players_away_team.nlargest(2, 'defending')['defending'].mean()
        ...
        X_predict.loc[idx_number, ['gk_compared']] = top_players_home_team[['goalkeeping_diving', 'goalkeeping_handling', 'goalkeeping_kicking',
                          'goalkeeping_positioning', 'goalkeeping_reflexes']].max(axis=0).mean() - top_players_away_team[['goalkeeping_diving', 'goalkeeping_handling', 'goalkeeping_kicking',
                          'goalkeeping_positioning', 'goalkeeping_reflexes']].max(axis=0).mean()
...
for idx, prediction in enumerate(svcclass_predictions):
    ...
    if svcclass_predictions[idx_count] == -1:
        print(match_df.loc[idx_count, 'Home Team'], ' wins with {}/{} goals'.format(svrgoals_predictions[idx], lgbmgoals_predictions[idx]))
